...
description: 'Shiny.AiConversation wraps chat clients, speech recognition, text-to-speech, audio playback, message persistence, and state management into a single IAiConversationService interface for .NET.'
pubDate: 'May 06 2026'
heroImage: '../../../../assets/ai-conversation-hero.svg'
tags: ['Shiny', '.NET', 'AI', 'MAUI', 'Blazor']
---

> **Early Beta** — This library is in active development. APIs may change, and some features are still being stabilized. Feedback and bug reports are very welcome, but don't ship it to production just yet.
...
That's it.  The service registers `IAiConversationService` with all the wiring — speech services from [Shiny.Speech](https://shinylib.net/speech), chat completions from [Microsoft.Extensions.AI](https://www.nuget.org/packages/Microsoft.Extensions.AI), audio playback, time provider, and optional message persistence.  The default `IChatClientProvider` resolves `IChatClient` straight from DI, so for most apps you just register your chat client and go.  For advanced scenarios (on-demand auth, token refresh), you can still implement `IChatClientProvider` directly.
...
This is what powers the "Aura" visualization in the sample app — a pulsing orb that changes color based on what the AI is doing.
...
- [Documentation](https://shinylib.net/aiconversation/)
...
The library is MIT licensed and open source.  I'd love to hear what you build with it.
