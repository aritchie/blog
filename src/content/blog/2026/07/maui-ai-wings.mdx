...
description: 'A .NET MAUI app built straight on Microsoft.Extensions.AI — an IChatClient onto OpenAI, ChatOptions.Tools, and UseFunctionInvocation running the loop. The chat UI is a control; the hands are Shiny — contacts, reminders, GPS, health, music, a local document DB, your own services, and the app''s own navigation. All opt-in, all switchable, all AOT-clean.'
pubDate: 'Jul 24 2026'
...
## Turning Health off properly

Most capabilities you turn off by deleting their registration line and their package reference — done. Health is the one that needs more care, because commenting out a line still leaves you carrying the NuGet packages and, on iOS, a HealthKit **entitlement you can't sign** without the right provisioning profile. So the sample gates the whole thing from MSBuild with a single switch:

```xml
<PropertyGroup>
    <EnableAppleHealth>true</EnableAppleHealth>
</PropertyGroup>
```

That one switch drives three things — packages, compiler constant, and the entitlement:

```xml
<ItemGroup Condition="'$(EnableAppleHealth)' == 'true'">
    <PackageReference Include="Shiny.Health" Version="2.0.0" />
    <PackageReference Include="Shiny.Health.Extensions.AI" Version="2.0.0" />
</ItemGroup>

<PropertyGroup Condition="'$(EnableAppleHealth)' == 'true'">
    <DefineConstants>$(DefineConstants);ENABLE_APPLE_HEALTH</DefineConstants>
</PropertyGroup>

<!-- HealthKit is an entitlement, which needs a provisioning profile that carries it.
     Only wire the entitlements file up when Health is actually on. -->
<PropertyGroup Condition="'$(EnableAppleHealth)' == 'true' AND
                          $([MSBuild]::GetTargetPlatformIdentifier('$(TargetFramework)')) == 'ios'">
    <CodesignEntitlements>Platforms/iOS/Entitlements.plist</CodesignEntitlements>
</PropertyGroup>
```

Set `EnableAppleHealth=false` and HealthKit is gone from the build entirely — the packages, the `ENABLE_APPLE_HEALTH`-gated block in `MauiProgram.cs`, and the entitlement — so a plain automatic provisioning profile just works. That's why it's the only capability behind a compiler constant: the rest don't carry an entitlement, so they never needed one.
